...
def main():
    ...
    parser.add_argument("--alphas_norm", type=str, default=None,
                        help="Normalized alpha values (alpha_norm = alpha * eff_penalty_mean)")
    parser.add_argument("--betas_norm", type=str, default=None,
                        help="Normalized beta values (beta_norm = beta * s_base_mean)")
    ...
    # Compute normalization factors from own neg docs
    all_overflow = []
    all_gap = []
    all_sbase = []
    for q_idx in range(n_queries):
        ...
        for ni in range(15):
            ...
            all_overflow.append(sn - tau)
            all_gap.append(sn - sb)
            all_sbase.append(sb)
    ...
    print(f"  eff_penalty_mean = {eff_penalty_mean:.4f}")
    print(f"  sbase_mean = {sbase_mean:.4f}")
    print(f"  Alpha normalization factor (1/eff_penalty_mean) = {1.0/eff_penalty_mean:.2f}")

    # Convert normalized parameters to actual parameters
    if args.alphas_norm:
        alphas_norm_list = [float(x) for x in args.alphas_norm.split(",")]
        alpha_list = [an / eff_penalty_mean for an in alphas_norm_list]
        print(f"  alphas_norm: {alphas_norm_list}")
        print(f"  -> alphas: {[f'{a:.3f}' for a in alpha_list]}")
    else:
        alpha_list = [0.1, 0.3, 0.5, 0.7, 1.0, 1.5, 2.0]

    if args.betas_norm:
        betas_norm_list = [float(x) for x in args.betas_norm.split(",")]
        beta_list = [bn / sbase_mean for bn in betas_norm_list]
        print(f"  betas_norm: {betas_norm_list}")
        print(f"  -> betas: {[f'{b:.3f}' for b in beta_list]}")
    else:
        ...
        for beta in beta_list:
            for delta in delta_list:
                for gamma in gamma_list:
                    ...
                    alpha_norm = alpha * eff_penalty_mean
                    beta_norm = beta * sbase_mean
                    all_results.append(
                        {
                            "alpha": float(alpha),
                            "beta": float(beta),
                    ...
                    if trial % 50 == 0:
                        print(
                            f"[{trial}/{total}] a={alpha:.3f}(n={alpha_norm:.4f}) b={beta:.3f}(n={beta_norm:.4f}) d={delta} g={gamma} | MAP={avg_map:.4f} nDCG@5={avg_ndcg:.4f}"
                        )

    all_results.sort(key=lambda x: x["map"], reverse=True)
    print(f"\n=== Top 10 by MAP (normalized, {n_dist} distractors, {model_name}) ===")
    for i, r in enumerate(all_results[:10]):
        print(
            f'{i+1}. a={r["alpha"]:.3f}(n={r["alpha_norm"]:.4f}) b={r["beta"]:.3f}(n={r["beta_norm"]:.4f}) d={r["delta"]:.2f} g={r["gamma"]:.1f} | MAP={r["map"]:.4f} nDCG@5={r["ndcg@5"]:.4f}'
        )

    all_results.sort(key=lambda x: x["ndcg@5"], reverse=True)
    print(f"\n=== Top 10 by nDCG@5 (normalized, {n_dist} distractors, {model_name}) ===")
    for i, r in enumerate(all_results[:10]):
        print(
            f'{i+1}. a={r["alpha"]:.3f}(n={r["alpha_norm"]:.4f}) b={r["beta"]:.3f}(n={r["beta_norm"]:.4f}) d={r["delta"]:.2f} g={r["gamma"]:.1f} | MAP={r["map"]:.4f} nDCG@5={r["ndcg@5"]:.4f}'
        )
    ...
    with open(
        f"dataset/FollowIR_train/train/train_param_search_normalized{n_dist}_{model_name_safe}{suffix}.json",
        "w",
    ) as f:
        json.dump(
            {
                ...
                "eff_penalty_mean": float(eff_penalty_mean),
                "sbase_mean": float(sbase_mean),
