...
def main():
    ...
    parser.add_argument("--sampling_strategy", type=str, default="uniform",
                        choices=["uniform", "sbase_calibrated", "at_risk_calibrated"],
                        help="uniform: original uniform sampling; "
                             "sbase_calibrated: sample distractors to match test-set S_base distribution; "
                             "at_risk_calibrated: sample distractors to achieve target at-risk ratio")
    parser.add_argument("--target_at_risk_ratio", type=float, default=0.05,
                        help="Target at-risk ratio for at_risk_calibrated strategy")
    ...
    elif args.sampling_strategy == "sbase_calibrated":
        # Simulate test-set S_base distribution: most docs have low S_base
        # Use exponential sampling: prefer docs with lower S_base
        distractor_indices = []
        for q_idx in range(n_queries):
            own_neg_start = q_idx * 15
            candidates = np.array([
            ...
            # Invert: lower S_base -> higher probability
            # Use softmax with negative temperature on S_base
            weights = np.exp(-10.0 * s_base_vals)
            weights /= weights.sum()
            sampled = np.random.choice(candidates, size=n_dist, replace=False, p=weights)
            distractor_indices.append(sampled)

    elif args.sampling_strategy == "at_risk_calibrated":
        # Target: achieve a specific at-risk ratio among distractors
        # At-risk = S_neg > S_base for the distractor doc
        # Strategy: mix "safe" docs (S_neg < S_base) and "at-risk" docs (S_neg > S_base)
        # to achieve target_at_risk_ratio
        target_ratio = args.target_at_risk_ratio
        distractor_indices = []
        for q_idx in range(n_queries):
            own_neg_start = q_idx * 15
            candidates = np.array([
            ...
            s_base_vals = S_base_neg_matrix[q_idx, candidates].numpy()
            s_neg_vals = S_neg_neg_matrix[q_idx, candidates].numpy()
            is_at_risk = s_neg_vals > s_base_vals

            safe_indices = candidates[~is_at_risk]
            risky_indices = candidates[is_at_risk]

            n_risky_target = int(n_dist * target_ratio)
            n_safe_target = n_dist - n_risky_target

            if len(risky_indices) >= n_risky_target and len(safe_indices) >= n_safe_target:
                sampled_risky = np.random.choice(risky_indices, size=n_risky_target, replace=False)
                sampled_safe = np.random.choice(safe_indices, size=n_safe_target, replace=False)
                sampled = np.concatenate([sampled_risky, sampled_safe])
            elif len(safe_indices) >= n_safe_target:
                sampled = np.random.choice(safe_indices, size=n_dist, replace=False)
            else:
                sampled = np.random.choice(candidates, size=n_dist, replace=False)
    ...
    print(f"\n=== Top 10 by MAP ({args.sampling_strategy}, {n_dist} distractors, {model_name}) ===")
    ...
    print(f"\n=== Top 10 by nDCG@5 ({args.sampling_strategy}, {n_dist} distractors, {model_name}) ===")
    for i, r in enumerate(all_results[:10]):
        print(
            f'{i+1}. a={r["alpha"]:.1f} b={r["beta"]:.1f} d={r["delta"]:.2f} g={r["gamma"]:.1f} | MAP={r["map"]:.4f} nDCG@5={r["ndcg@5"]:.4f}'
        )
    ...
    with open(
        f"dataset/FollowIR_train/train/train_param_search_{args.sampling_strategy}{n_dist}_{model_name_safe}{suffix}.json",
        "w",
    ) as f:
        json.dump(
            {
                ...
                "target_at_risk_ratio": args.target_at_risk_ratio,
